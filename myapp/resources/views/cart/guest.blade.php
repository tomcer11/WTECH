        ...
        @foreach ($cart as $id => $product)
            <div class="card mb-3 cart">
                <div class="row g-0 align-items-center">
                    <div class="col-sm-3">
                    <img src="{{asset('storage/'.$product['product_image_name'])}}" class="img-fluid rounded-start" alt="$cart_item->images[0]->alt_text">
                    </div>
                    <div class="col-9">
                        <div class="card-body">
                            <div class="row align-items-center justify-content-between"> 
                                <div class="col-sm-3 col-9 order-sm-1 order-2 me-auto">
                                    <h5 class="card-title">{{$product['product_producer']}}</h5>
                                    <p class="card-text text-secondary">{{$product['product_model']}}</p>
                                </div>
                                ...
                                <div class="col-9 col-sm-1 order-sm-4 order-1">
                                    <form action="{{ url('cart', [$id, $product['product_count']]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
